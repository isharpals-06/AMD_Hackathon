...
class RoutingEngine:
    ...
    def _load_rules(cls) -> tuple[dict, dict]:
        ...
        if not os.path.exists(yaml_path):
            logger.warning(f"Routing rules config not found at {yaml_path}. Using hardcoded fallbacks.")
        ...
        try:
            ...
            logger.info("Loaded routing rules and model pricing from configs/routing_rules.yaml successfully.")
    ...
    def get_routing(cls, task_type: str) -> dict:
        ...
        if task_type == "math":
            routing = {**routing, "primary_model": config.MATH_PRIMARY_MODEL or routing.get("primary_model"), "fallback_model": config.MATH_FALLBACK_MODEL or routing.get("fallback_model")}
        elif task_type == "coding":
            routing = {**routing, "primary_model": config.CODING_PRIMARY_MODEL or routing.get("primary_model"), "fallback_model": config.CODING_FALLBACK_MODEL or routing.get("fallback_model")}
        elif task_type == "research":
            routing = {**routing, "primary_model": config.RESEARCH_PRIMARY_MODEL or routing.get("primary_model"), "fallback_model": config.RESEARCH_FALLBACK_MODEL or routing.get("fallback_model")}
        elif task_type == "casual_chat":
            routing = {**routing, "primary_model": config.CASUAL_PRIMARY_MODEL or routing.get("primary_model"), "fallback_model": config.CASUAL_FALLBACK_MODEL or routing.get("fallback_model")}
        else:
            routing = {**routing, "primary_model": config.CASUAL_PRIMARY_MODEL or routing.get("primary_model"), "fallback_model": config.CASUAL_FALLBACK_MODEL or routing.get("fallback_model")}
